...
def create_dictionary(df, no_below=5, no_above=0.2):
    # reference: https://qiita.com/youyouyou/items/b2fa94af74c583c9d841
    d = corpora.Dictionary(__docs(df))
...
def create_corpus(df, dic, test_ratio=0.1):
    corpus = [dic.doc2bow(w) for w in __docs(df)]
